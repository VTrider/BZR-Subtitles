...
void gui::SetupMenu(LPDIRECT3DDEVICE9 d3dDevice) noexcept
{
	...
	float scaledFontSize = std::floor(screenHeight / 30.0f);
	// bzoneFont = io.Fonts->AddFontFromFileTTF(R"(C:\Program Files (x86)\Steam\steamapps\common\Battlezone 98 Redux\addon\BZONE.ttf)", scaledFontSize, NULL, io.Fonts->GetGlyphRangesJapanese());
	bzoneFont = io.Fonts->AddFontFromMemoryCompressedTTF(font_compressed_data, font_compressed_size, scaledFontSize, nullptr, io.Fonts->GetGlyphRangesJapanese());
